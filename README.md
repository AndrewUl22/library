...
\- Access rights for the `base.group\_user` group (regular internal user)
...
\- Add an `is\_overdue` computed field based on `due\_date`
...
\- No loan-period limit (`due\_date` isn't filled in automatically)
